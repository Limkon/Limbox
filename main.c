...
typedef struct { char host[256]; int port; char path[256]; char sni[256]; char user[128]; char pass[128]; } ProxyConfig;
...
typedef struct { int first_packet_sent; } FragCtx;
...
bytes_sent += ret;
remaining -= ret;
if (g_fragDelayMs > 0) { Sleep(rand() % (g_fragDelayMs + 1)); }
}
return bytes_sent;
}
